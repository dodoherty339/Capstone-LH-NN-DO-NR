    ...
    class DisplayResultsTable
    {
        ...
        public static void displayTable()
        {
            ...
            else
            {
                ...
                Price_Comparison.PriceComparison.form.lblDisplayedRecords.Text = "0-0 of 0";
            }
        }
        ...
        public static void displayRow(int rowNum, DataRow row)
        {
            ...
            Price_Comparison.PriceComparison.form.tblResults["DatabasePrice", rowNum].Value = String.Format("{0:$0.00}", (Double)row["dbPrice"]);

            if ((Double)row["onlinePrice"] == 0)
            {
                Price_Comparison.PriceComparison.form.tblResults["OnlinePrice", rowNum].Value = "N/A";
                Price_Comparison.PriceComparison.form.tblResults["Difference", rowNum].Value = "-";
                row["diff"] = 0;
            }
            else
            {
                Price_Comparison.PriceComparison.form.tblResults["OnlinePrice", rowNum].Value = String.Format("{0:$0.00}", (Double)row["onlinePrice"]);
            }
                
            if ((Double)row["diff"] < 0)
            {
                Price_Comparison.PriceComparison.form.tblResults["Difference", rowNum].Style.BackColor = System.Drawing.Color.Tomato;
                Price_Comparison.PriceComparison.form.tblResults["Difference", rowNum].Value = String.Format("{0:0.00}", (Double)row["diff"]);
            }
            if ((Double)row["diff"] > 0)
            {
